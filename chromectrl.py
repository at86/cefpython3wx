...
def CreateMessageLoopTimer(obj, timerMillis):
    ...
    # atdo for wx 4, new version, but I can't find how to use wx 4.
    # g_messageLoopTimer = wx.Timer()
    # atdo use wxPython 4, shell be undercode
    g_messageLoopTimer = wx.Timer(obj,1)
    obj.Bind(wx.EVT_TIMER, MessageLoopTimer, g_messageLoopTimer)
    g_messageLoopTimer.Start(timerMillis)
    Debug("g_messageLoopTimer.GetId() = " + str(g_messageLoopTimer.GetId()))
...
class ChromeWindow(wx.Window):
    ...
    def __init__(self, parent, url="", useTimer=True,
        ...
        if useTimer:
            CreateMessageLoopTimer(self, timerMillis)
        else:
